...
function buildDecision(eth1h, eth4h, eth15m, btc, ticker) {
  ...
  if (canBuy) {
    ...
    action = 'NO COMPRAR';
    cardClass = 'no-buy';
    reason = `Debilidad: ${noBuyReasons.slice(0, 3).join(', ')}.`;
  ...
  if (action === 'COMPRAR') {
    ...
    if (extended || rsiHigh) {
      plan = `Esperar retroceso hacia EMA21: zona aproximada ${formatPrice(zoneLow)} - ${formatPrice(eth1h.ema21Last * 1.006)}. Precio extendido ahora.`;
    } else {
...
function renderDecision(decision) {
  els.decisionCard.className = `decision-card ${decision.cardClass}`;
  const icon = decision.action === 'COMPRAR' ? '🟢' : decision.action === 'NO COMPRAR' ? '🔴' : '🟡';
...
function showError(error) {
  ...
  els.decisionCard.className = 'decision-card no-buy';
  els.decisionText.textContent = '🔴 NO COMPRAR';
